...
getNemenyiCD <- function (alpha = 0.05, 
    ...
    #' @title Nemenyi Critical Difference values
    #'
    #' @description: Auxiliar function to compute the critical difference values after Nemenyi test
    #'
    #' @parm alpha num,        Alpha value for the test
    #' @parm num.alg num,      Number of algorithms tested
    #' @parm num.problems num, Number of problems where the algorithms have been tested
    #'
    #' @returns cd num,        Corresponding critical difference
    #'
    #' Reference:
...
plot_Critical_Difference_color <- function (
        results.matrix, 
        alpha = 0.05, 
        cex   = 0.75, 
    ...
    #'
    #' @description Auxiliar function to plot the critical difference plots. Taken from `github.com/cran/scmamp`
    #'
    #' @param matrix results.matrix,    Matrix with the performance results, 
    #'                                        with N evaluations (rows) and k methods (cols)  
    #' @param num alpha,                Alpha value for the test
    #' @param num cex,                  `cex` value to render `CD value` text
    #' @param vector colPalette,        Color palette for the k methods (cols)
    #' @param num side_marging,         Marging value for the plot's width
    #' @param num line.spacing,         Line spacing to render methods' labels
    #' @param num labels.cex,           `cex` value to render methods' labels
    #'
    #' Reference:
    #'   - Nemenyi, P. B. Distribution-Free Multiple Comparisons. Ph.D. thesis, Princeton University, 1963. 
    #'
    #' Modified from: 
    #'  https://github.com/cran/scmamp/blob/master/R/plotting.R
